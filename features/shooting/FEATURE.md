...
持球时按住射门键会进入可取消的蓄力窗口，松开或蓄力满后才播放踢球动画并在触球帧出球。蓄力期间保持带球状态，方向输入可以改变带球方向但不会取消蓄力；释放时当前方向会进一步微调球门上下轨迹。

### WE2000 对照

采用 WE2000/PES 的“输入蓄力 -> 起手动画 -> 触球出球”节奏。射门方向自动吸附目标球门；蓄力期间球员仍可带球变向，释放时的方向输入再微调目标线路。射门属性、距离和蓄力共同决定最终速度。
...
- [x] 蓄力期间方向键不会取消射门，可改变球员带球方向，球保持 `DRIBBLING`。
...
- `ShootingPhysics` 计算 0.75 秒上限的蓄力比例、球门瞄准和射门速度，并根据 technique、shooting、power 调整目标线路。
- `PlayerStatePreppingShot` 不再播放 `prep_kick` 或清零速度；蓄力期间继续处理方向移动。
- `PlayerStateShooting` 只负责踢球动画和触球回调，并清理玩家蓄力显示状态。
- 速度范围为 90..700 px/s；整体力量倍率为 1.2；射门属性提供 0.78..1.18 倍修正；距离提供 0.75..1.35 倍自动补偿。
...
自动化覆盖边界、球门辅助方向、蓄力单调性和属性差异。独立手动场景验证从带球进入蓄力、蓄力中变向、动画触球、球门命中和属性差异。
