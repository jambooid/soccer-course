...
持球时按住射门键会进入可取消的蓄力窗口，松开或蓄力满后才播放踢球动画并在触球帧出球。蓄力期间保留带球速度；蓄力开始后新产生的方向输入会取消射门并回到带球，避免球员抬脚后呆立。

### WE2000 对照

采用 WE2000/PES 的“输入蓄力 -> 起手动画 -> 触球出球”节奏。射门方向自动吸附目标球门；蓄力期间的方向输入优先保留带球控制并取消射门。射门属性、距离和蓄力共同决定最终速度。

## 2. 成功标准

- [x] 蓄力比例在 0..1 内，力量随蓄力单调增加并有上限。
- [x] 射门方向自动吸附目标球门，并会依据属性、距离和蓄力调整力量。
- [x] 蓄力前按方向键回到 `MOVING`，球保持 `DRIBBLING`，不播放踢球动画。
...
- `ShootingPhysics` 计算 1.5 秒上限的蓄力比例、球门瞄准和射门速度。
- `PlayerStatePreppingShot` 不再播放 `prep_kick` 或清零速度；方向输入取消蓄力。
...
自动化覆盖边界、球门辅助方向、蓄力单调性和属性差异。独立手动场景验证从带球进入蓄力、方向取消、动画触球、球门命中和属性差异。
